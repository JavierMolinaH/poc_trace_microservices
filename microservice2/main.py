...
app = FastAPI()
logger = get_logger("microservice2")
tracer = init_tracer(service_name="microservice2")
...
def index(request: Request):
    ...
    logger.info(f"Microservice2 received call with operation_id={operation_id}")

    headers = {"X-Operation-Id": operation_id}
    try:
        r = requests.get("http://localhost:8003/", headers=headers)
        return r.json()
    except Exception as e:
        logger.error(f"Error calling microservice3: {e}")
        return JSONResponse(status_code=500, content={"error": str(e)})


if __name__ == "__main__":
    uvicorn.run("microservice2.main:app", host="0.0.0.0", port=8002, reload=True)
